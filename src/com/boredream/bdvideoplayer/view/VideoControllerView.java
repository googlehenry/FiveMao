...
public class VideoControllerView extends FrameLayout {
    ...
    private MediaPlayer splitMediaPlayer;
    ...
    private void initControllerPanel() {
        ...
        mErrorView.setOnVideoControlListener(new SimpleOnVideoControlListener() {
            ...
            public void onRetry(int errorStatus) {
            	if(splitMediaPlayer!=null){
            		if(onVideoControlListener != null) {
                        onVideoControlListener.onRetry(errorStatus);
...
//            }
    ...
    private void hide() {
        ...
        //commented out by Henry at 2018/03/06
//        if (!DisplayUtils.isPortrait(getContext())) {
//            // 横屏才消失
//            mControllerBack.setVisibility(GONE);
//        }
    ...
    private final Runnable mShowProgress = new Runnable() {
        ...
        public void run() {
            ...
            if(splitMediaPlayer!=null){
            	try{
		            if (!mDragging && mShowing && splitMediaPlayer.isPlaying()) {
		                postDelayed(mShowProgress, 1000 - (pos % 1000));
		            }
            	}catch(Exception e){
            		postDelayed(mShowProgress, 1000 - (pos % 1000));
    ...
    private int splitBaseTime = 0;
    private int splitTotalTime = 0;
    ...
    private int setProgress() {
        ...
        if (mPlayer == null && splitMediaPlayer == null) {
            return 0;
        }
        if(splitTotalTime>0){
        	try{
        		...
        		if(splitMediaPlayer!=null){
        			currentPos = splitMediaPlayer.getCurrentPosition();
		            duration = splitMediaPlayer.getDuration();
		            percent = (int) (((double)currentPos)/((double)duration));
        		}
	            
	            int currentBufPos = percent * duration / 100;
	            
	            long splitPos = splitBaseTime + currentPos;
	            long splitBufPos = splitBaseTime + currentBufPos;
	            
	            if (mPlayerSeekBar != null) {
	                if (splitPos > 0) {
	                    // use long to avoid overflow
	                    long pos = 1000L * splitPos / splitTotalTime;
	                    mPlayerSeekBar.setProgress((int) pos);
	                }
	                
	                long bufPos = 1000L * splitBufPos / splitTotalTime;
	                mPlayerSeekBar.setSecondaryProgress((int)bufPos);
	                
	            }
	
	            mVideoProgress.setText(StringUtils.stringForTime((int)splitPos));
	            mVideoDuration.setText(StringUtils.stringForTime(splitTotalTime));
    ...
    public void checkShowError(boolean isNetChanged) {
        ...
        if (isConnect) {
            ...
            if(mPlayer!=null){
            	mPlayer.pause();
            }else if(splitMediaPlayer!=null){
            	splitMediaPlayer.pause();
            }
    ...
    VideoDuoPlayerActivity duoPlayerActivity;
    
    public VideoDuoPlayerActivity getDuoPlayerActivity() {
		return duoPlayerActivity;
	}
	...
	private final SeekBar.OnSeekBarChangeListener mSeekListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar bar, int progress, boolean fromuser) {
            ...
            if(splitTotalTime>0){
                mDraggingProgress = (splitTotalTime / 1000L * progress);
                ...
                if(mPlayer.getBufferPercentage()*duration/100 < mDraggingProgress ){
                	mDraggingProgress = mPlayer.getBufferPercentage()*duration/100;
                }
        ...
        public void onStopTrackingTouch(SeekBar bar) {
        	int minisecsInput = (int) mDraggingProgress;
        	if(splitTotalTime>0){
        		if(duoPlayerActivity!=null){
        			duoPlayerActivity.dragtoSplitTimePoint(minisecsInput);
        			toggleDisplay();
        		}
	            mDragging = false;
	            mDraggingProgress = 0;
    ...
    private void doPauseResume() {
    	if(mPlayer!=null){
	        ...
	        if(splitMediaPlayer!=null && splitMediaPlayer.isPlaying()){
	        	pause();
	        }else{
	        	play();
	        }
...
//            mControllerBack.setVisibility(VISIBLE);
	...
	public int getSplitTotalTime() {
		return splitTotalTime;
	}

	public void setSplitTotalTime(int splitTotalTime) {
		this.splitTotalTime = splitTotalTime;
	}

	public int getSplitBaseTime() {
		return splitBaseTime;
	}

	public void setSplitBaseTime(int splitBaseTime) {
		this.splitBaseTime = splitBaseTime;
	}

	public MediaPlayer getSplitMediaPlayer() {
		return splitMediaPlayer;
	}

	public void setSplitMediaPlayer(MediaPlayer splitMediaPlayer) {
